...
public class ScriptExecutorSnippet {
    private static final Logger logger = LoggerFactory.getLogger(ScriptExecutorSnippet.class);
    ...
    public void execute(String command) throws Exception {
        ...
        if (exitVal == 0) {
            logger.debug("命令脚本[" + command + "]执行正常结束!!");
        } else {
            logger.debug("命令脚本[" + command + "]执行出现错误!! 返回值=[" + exitVal + "]");
            throw new Exception(
                "命令脚本[" + command + "]执行出现错误!! 返回值=[" + exitVal + "]");
        }
